...
{
    public static bool SaveGame(SaveGameFile saveGame, string name)
    {
    ...
    public static SaveGameFile LoadGame(string name)
    {
        ...
        using (FileStream stream = new FileStream(GetSavePath(name), FileMode.Open))
        {
            try
            {
                return formatter.Deserialize(stream) as SaveGameFile;
            }
            catch (Exception)
            {
                return null;
